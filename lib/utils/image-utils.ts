...
export const myImageLoader = ({ src, width, quality }) => {
  return `${APP_ENV.DOMAIN}/cdn-cgi/image/w=${width},q=${quality || 90},format=webp/${src}`
}
...
export const generateImageUrl = (src: string, origin: "auto" | "cloud" | "internal" | "external" = "auto") => {
  ...
  switch (origin) {
    ...
    case "auto": {
      ...
      // from cloud
      if (src.match(/^candy\/image\/.+/)) return `${APP_ENV.MEDIA_BASE}/${src}`
